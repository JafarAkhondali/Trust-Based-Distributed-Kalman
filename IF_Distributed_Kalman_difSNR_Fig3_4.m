% Trust-Based Distributed Kalman - Different SNRs for Different Nodes
...
Q = 0.1*eye(p_state); % variance of states
...
% 0: without attack (different SNR) 1: FDI attack, 2: Reply attack, 3: Combined attack
p_type = 0; % or 2
...
rmse_smooth_It = zeros(size(p_SNR,2),p_num); % Uniform weights
rmse_smooth2_It = zeros(size(p_SNR,2),p_num); % Uniform weights
rmse_smooth3_It = zeros(size(p_SNR,2),p_num); % Uniform weights

for itd  = 1:size(p_SNR,2)
    
    snrS = [p_SNR(itd)*ones(1,p_noisy_num) 10*ones(1,p_nodes-p_noisy_num)] + 0.01*rand(1,p_nodes);

% 1. parameter of FDI attack
    attackMean = p_FDI(1,1)*ones(1,p_nodes).*Boolean; % amplitude for FDI attack
    attackStdDev = p_FDI(1,2)*ones(1,p_nodes).*Boolean; % amplitude for FDI attack
...
for it = 1:size(rmse_IT,1)
        ...
        plot(px,rmse_IT(it,:),LineStyle{itL},'Color',ColorSpec{itC},'LineWidth',2);
                if it<size(p_SNR,2)+1
                    strP{index}=['Uniform [55]: SNR = ',num2str(p_SNR2(it)),' dB'];
                elseif it <2*size(p_SNR,2)+1
                    strP{index}=['Relative [56]: SNR = ',num2str(p_SNR2(it)),' dB'];
                elseif 2*size(p_SNR,2) < it
                    strP{index}=['Trust: SNR = ',num2str(p_SNR2(it)),' dB'];
                end
        hold on
...
for it = 1:p_nodes
        ...
        strP{index}=['Node 02: SNR = ',num2str(p_SNR(1,end)),' dB'];
